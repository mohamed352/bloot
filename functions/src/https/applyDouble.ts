...
export const applyDouble = functions.https.onCall(async (request) => {
  if (!request.auth) {
  ...
  if (!gameId || !action || !['double', 'pass'].includes(action)) {
  ...
  return db.runTransaction(async (transaction) => {
    ...
    if (!state.awaitingDouble || !state.doubling || state.doubling.turn !== seatIndex) {
      throw new functions.https.HttpsError('failed-precondition', 'Not time to double');
    }

    engine.applyDouble(match, seatIndex, action);
