...
void main() {
  ...
  group('watchStream real-time updates', () {
    blocTest<DiscoverCubit, DiscoverState>(
      ...
      build: () {
        ...
        when(() => discoverRepository.watchStream('s1'))
    ...
    blocTest<DiscoverCubit, DiscoverState>(
      ...
      build: () {
        final streamController = StreamController<DiscoverStream>();
        final chatController = StreamController<List<StreamChatMessage>>();
        addTearDown(() async {
  ...
  group('findStreamByCode', () {
    test('delegates to repository and returns result', () async {
      when(() => discoverRepository.findStreamIdByCode('ABC123'))
    ...
    test('returns null when no stream matches', () async {
      when(() => discoverRepository.findStreamIdByCode('NOPE'))
  ...
  group('loadStream re-entrancy', () {
    blocTest<DiscoverCubit, DiscoverState>(
      ...
      build: () {
        when(() => discoverRepository.getStreamById('s1'))
            .thenAnswer((_) async => stream);
        when(() => discoverRepository.getStreamById('s2'))
            .thenAnswer((_) async => testStream(id: 's2', title: 'Other'));
        when(() => discoverRepository.watchStream('s1'))
            .thenAnswer((_) => const Stream.empty());
        when(() => discoverRepository.watchStream('s2'))
            .thenAnswer((_) => const Stream.empty());
        when(() => discoverRepository.watchStreamChat('s1'))
            .thenAnswer((_) => const Stream.empty());
        when(() => discoverRepository.watchStreamChat('s2'))
            .thenAnswer((_) => const Stream.empty());
        return buildCubit();
      },
      act: (cubit) async {
  ...
  group('selectFilter does nothing in streamLoaded state', () {
    blocTest<DiscoverCubit, DiscoverState>(
      ...
      build: buildCubit,
      seed: () => DiscoverState.streamLoaded(stream: stream, messages: const []),
