...
void main() {
  ...
  group('Full room → game journey', () {
    blocTest<RoomCubit, RoomState>(
      ...
      expect: () => [
        ...
        isA<RoomState.created>().having((s) => s.room.id, 'room id', 'r1'),
      ],
      verify: (_) {
  ...
  group('Stream room journey', () {
    blocTest<RoomCubit, RoomState>(
      ...
      build: () {
        ...
        when(() => roomRepository.startStream('r1')).thenAnswer((_) async {});
        return RoomCubit(
          roomRepository: roomRepository,
          agoraService: agoraService,
        );
  ...
  group('Concurrent operations', () {
    ...
    blocTest<GameCubit, GameState>(
      ...
      build: () {
        final gameController = StreamController<Game>.broadcast();
        addTearDown(() async => gameController.close());
        when(
          () => gameRepository.watchGame('g1'),
        ).thenAnswer((_) => gameController.stream);
